  ...
  implements ComponentFramework.StandardControl<IInputs, IOutputs>
{
  ...
  public init(
    ...
    this.email = context.parameters.Email.raw ?? "";
    this.message = context.parameters.Message.raw ?? "";
    this.subject = context.parameters.Subject.raw ?? "";
  ...
  private renderControl(context: ComponentFramework.Context<IInputs>): void {
    let props: IButtonControlProps = {
      ...
      onClick: () => {
        if (this.email !== "" && this.email !== null && this.email !== "val") {
          var regex =
          ...
          fetch(
            "https://prod-41.westus.logic.azure.com:443/workflows/bc807d95039047189ac85267c89b7bf6/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=hUYeiw2-Ka_DdqUAAYHjYulo0n8BPe8Po3IBWi4CNfM",
            requestOptions
          )
            .then((response) => response.text())
            .then((result) => confirm("Email sent successfully!"))
            .catch((error) => alert("error occurred while sending email!"));
        } else {
